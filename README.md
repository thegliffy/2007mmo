...
You walk a tiny woodland hamlet, wave at other people, pick brambleberries and hazel nuts, crush berries at the millstone, bake a hearth tart or roast a nut, and talk in a parchment log. The server is authoritative on a **600ms tick**. The pack lives in **Postgres**, so closing the tab (or killing the world container) does not mint extra berries.
...
This tree is the Week 1 cut of Phase 2. It keeps the PoC tick world and adds four things the planner asked for — nothing from the later wishlist.
...
**Camera:** Week 1 still paints a **top-down** hamlet. A ¾ view would be a real renderer change; that is a follow-up, not this PR.

**Out of scope (still):** combat, character creator, multi-world, market, quests, PvP, Kubernetes.
...
- Click a tart or roast in the pack to eat it. Whole berries will not bake; the hearth says so.
...
3. Click-to-move (WASD optional) on a small tile map. Other players and three wandering villagers are visible (naive AOI: the whole hamlet).
...
Open [http://127.0.0.1:8080](http://127.0.0.1:8080) in two desktop tabs. Enter names. Click the grass to walk, a bramble or hazel to gather, the millstone to crush, the hearth to cook. Type in the parchment log.
...
npm run stats
npm run metrics
npm test           # Go unit tests (tick, gather, mill, roast, no-dupe recovery)
...
- **Use:** click a hearth tart or roast hazel in the pack.
...
internal/world     tick, map, path, woodland work, pack
...
Full skill tree, market, quests, PvP, multi-world routing, 1k CCU claims, mobile polish, Agones/K8s, borrowed studio content.
